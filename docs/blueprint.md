...
- **MUI (Material-UI)**: UIコンポーネントライブラリとしてMUIを全面的に採用する。
  - **スタイルの一元管理**: CSS in JS（`sx` prop）を用いて、コンポーネント単位でスタイルを閉じる。グローバルなスタイルは`ThemeProvider`と`CssBaseline`で管理し、`*.css`ファイルは原則として使用しない。
  - **テーマ**: アプリケーション全体の色、タイポグラフィ、スペーシングは`theme.ts`で定義し、一貫性を保つ。
